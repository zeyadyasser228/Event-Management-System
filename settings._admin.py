...
def update_password():
    ...
    changepassword.execute("SELECT * FROM admin WHERE user_name=? AND password=?", (username, oldpass))
    user = changepassword.fetchone()

    if user:
        # If user exists, update the password
        changepassword.execute("UPDATE admin SET password=? WHERE user_name=? AND password=?", (newpass, username, oldpass))
        conn.commit()
        conn.close()
...
Apply_button.place(x=225, y=380)

Remember_Button = ctk.CTkButton(loginFrame, fg_color="#7B9976", text_color="#262724", text='Click to generate password',hover_color="#9CBB9F",
